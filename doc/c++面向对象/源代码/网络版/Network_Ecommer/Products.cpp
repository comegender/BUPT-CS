...
void Products::chooseChangeProductMode(string x, string userName, int insert,string New) {
	if (x == "0") return;
	else if (x == "1") {
		if (New.length() == 0 || New.find(' ') != string::npos) {
			cout << "名字不能是空，不能有空格\n";
			system("pause");
			return;
		}
		psheet[insert].changePname(New);
	}
	else if (x == "2") {
		// 检查输入是否合法
		bool isValid = true;
		...
		// 转换为 double 并检查是否为 0
		double money = stod(New);
		if (money <= 0) {
			cout << "充值金额必须大于 0！\n";
			system("pause");
			return;
		}
		psheet[insert].changePrice(money);
	}
	else if (x == "3") {
		psheet[insert].changeType(New);
		...
		psheet[insert].changeDescribe(New);
	}
	else if (x == "5") {
		psheet[insert].changeRemain(stoll(New));
	}
}

int Products::changeProduct(string userName, string x, string pname, string type, string New) {
		if (pname == "q") return 0;
		...
		for (int i = 0; i < belong_key[belong].size(); i++) {
			if (psheet[belong_key[belong][i]].getPname() == pname && psheet[belong_key[belong][i]].getType() == type) {
					//cout << "---------------------------------------------------------\n1:修改商品名\n2:修改价格\n3:修改商品类别\n4:修改商品描述\n5:修改商品数量\n0:退出\n---------------------------------------------------------\n";
				chooseChangeProductMode(x, userName, belong_key[belong][i], New);
				if (x == "0") return 0;
			}
			else if (i == belong_key[belong].size() - 1) { 
				//cout << "该商品不存在!\n"; 
				return 2;
			}
		}
...
void Products::chooseSearchMode(string x) {
	if (x == "0") return;
		...
		for (int i = 0; i < pname_key[spname].size(); i++) {
			...
			cout << "价格: " << psheet[pname_key[spname][i]].getPrice() << endl;
			if (IS_DISCOUNT) cout << "折后价: " << psheet[i].getNewPrice() << endl;
		...
		for (int i = 0; i < belong_key[sbelong].size(); i++) {
			...
			cout << "价格: " << psheet[belong_key[sbelong][i]].getPrice() << endl;
			if (IS_DISCOUNT) cout << "折后价: " << psheet[i].getNewPrice() << endl;
