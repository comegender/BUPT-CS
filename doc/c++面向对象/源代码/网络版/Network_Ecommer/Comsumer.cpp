...
int Comsumer::addCart(string cname, string pname,string type,string belong,ll count) {
	if (pname == "r") return 0;
	if (p.returnProductIfo(pname, type, belong).getType() == "") return 1;
...
int Comsumer::removeCart(string cname, string pname, string type, string belong, ll count) {
	if (pname == "r") return 0;
	productsIfo temp;
...
void Comsumer::showCart(string cname) {
	...
	for (int i = 0; i < shopping_cart[cname].size(); i++) {
		...
		if (IS_DISCOUNT) cout << "折后价: " << shopping_cart[cname][i].getNewPrice() << endl;
...
int Comsumer::changeCart(string cname, string pname, string type, string belong, ll count) {
	if (pname == "r") return 0;
	productsIfo temp;
	if (findCart(cname, pname, type, belong) == -1) {
		//cout << "未找到商品……\n";
